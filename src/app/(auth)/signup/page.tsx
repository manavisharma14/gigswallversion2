...
export default function SignUpPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (signInRes?.ok) {
          ...
          toast.error("Signup succeeded but login failed. Please sign in manually.");
          router.push("/signin");
        }
      } else {
  ...
  const PasswordMeter = () => (
    <div className="mt-1 text-xs">
      ...
      ></div>
      <p className="mt-1 text-gray-500">Password strength: {passwordStrength}</p>
    </div>
  );
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8 sm:p-10">
        ...
        <div className="text-center mb-6">
          ...
          <p className="mt-2 text-sm text-gray-500">
            Join <span className="font-semibold text-[#6B7FFF]">GigsWall</span> to find gigs or hire talent.
          </p>
        </div>

        {/* Toggle Student / Other */}
        <div className="flex bg-[#EDEBFF] rounded-full p-1 mb-8">
          {["Student", "Not a Student"].map((label, idx) => {
            const active = idx === 0 ? isStudent : !isStudent;
            return (
              <button
                key={label}
                type="button"
                onClick={() => setIsStudent(idx === 0)}
                className={`flex-1 py-2 rounded-full text-sm font-medium transition-all ${
                  active ? "bg-[#6B7FFF] text-white shadow-sm" : "text-[#4B3BB3]"
                }`}
              >
                {label}
              </button>
        ...
        {isStudent && (
          <div className="mb-6">
            <div className="h-1 bg-gray-200 rounded-full">
              <div
                className={`h-1 rounded-full transition-all ${
                  signupStep === 1 ? "w-1/2 bg-[#6B7FFF]" : "w-full bg-[#6B7FFF]"
                }`}
              />
            </div>
